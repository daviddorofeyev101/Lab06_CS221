...
public class ArraySet<T> implements SimpleSet<T> {
	...
	public boolean isEmpty() {
		if (rear > 0) { // add if-else statement to return correct values
            return false;
        } else {
            return true;
        }
	...
	public void add(T element) {
		if (!contains(element)) {
			...
			setArray[rear++] = element; // increment the rear
		}
	}
	...
	public T remove(T element) {
		...
		while (retVal == null && i < rear) {
			if (setArray[i].equals(element)) {
				...
				while (i < rear-1) { // in the condition (i<rear) causes out of bounds for array length, just subtract rear by 1
